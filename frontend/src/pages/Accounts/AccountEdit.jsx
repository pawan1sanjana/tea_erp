...
import { ArrowLeft, Edit, Shield, Mail, Lock, User, Camera, RefreshCw } from 'lucide-react';
...
export default function AccountEdit() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await apiClient.put(`/accounts/${id}`, formData);
      if (res.success) {
        navigate('/accounts');
      } else {
    ...
    <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
              <Edit className="text-tea-500" /> Edit User Profile
            </h2>
          </div>
        </div>
      </div>
      ...
      <div className="glass-panel p-8 rounded-3xl border border-slate-200 dark:border-slate-800">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Update Password <span className="text-xs font-normal text-slate-500">(Leave blank to keep current password)</span></label>
            <div className="relative">
